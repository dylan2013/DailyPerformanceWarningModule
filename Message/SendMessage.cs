...
namespace DailyPerformanceWarningModule
{
    public class SendMessage
    {
        ...
        private void BatchPushNotice()
        {
            ...
            eleDisplaySender.InnerText = "預警通知";
            ...
            {
                //發送對象
                var eleTarget = doc.CreateElement("TargetStudent");
            ...
            sb_log.AppendLine(string.Format("發送對象「{0}」", "學生"));

            sb_log.AppendLine(string.Format("發送標題「{0}」", _title));
            sb_log.AppendLine(string.Format("發送單位「{0}」", "預警通知"));
